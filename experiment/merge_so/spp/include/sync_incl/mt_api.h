 ...
 * @return  0 成功, -1 打开socket失败, -2 发送请求失败, -3 接收应答失败, -10 参数无效, 可打印errno, 
 */
 ...
 * @brief 采用随机端口的socket收发接口, 由socket来决定上下文, 业务来保证上下文
 *        [注意] UDP发送buff, 不推荐static变量, 有一定风险导致上下文错乱[重要]
 *        udp 发送前，调用connect绑定了远端地址，对端没有监听时，errno会返回
 *        ECONNREFUSED。
 * @param dst -请求发送的目的地址
 * @param pkg -请求包封装的包体
 * @param len -请求包封装的包体长度
 ...
 * @param buf_size -modify-接收应答包的buff大小, 成功返回时, 修改为应答包长度
 * @param timeout -超时时间, 单位ms
 * @return  0 成功, -1 打开socket失败, -2 发送请求失败, -3 接收应答失败, -4 链接失败, -10 参数无效, 可打印errno, 
 */
int mt_udpsendrcv_ex(struct sockaddr_in* dst, void* pkg, int len, void* rcv_buf, int& buf_size, int timeout);
...
ssize_t mt_write(int fd, const void *buf, size_t nbyte, int timeout, size_t &send_len);
...
/**
 * @brief 微线程timer函数
 * @param tm_sid timer的标识
 * @param tm_para timer的参数
 */
typedef void (*MtTimerFunc)(int tm_sid, void* tm_para);

/**
 * @brief 微线程timer创建函数
 * @param tm_func timer的执行函数
 * @param tm_para timer的参数
 * @param tm_interval timer延迟时间，单位:ms, <0,非法; =0，立刻执行； >0,延迟执行
 * @return >0 成功；返回timer的标识, <0 失败
 */
int mt_create_timer(MtTimerFunc tm_func, void* tm_para, uint32_t tm_interval);

/**
 * @brief 微线程timer销毁函数
 * @param tm_sid timer的标识
 * @return =0 销毁成功, <0 失败；-1, timer已执行完成，或者不存在；-2，timer正在执行
 */
int mt_destroy_timer(int tm_sid);
