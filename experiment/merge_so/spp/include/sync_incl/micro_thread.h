...
public:  
    ...
    static ssize_t write(int fd, const void *buf, size_t nbyte, int timeout, size_t &send_len);
    ...
    /**
     * @brief 微线程包裹的系统IO函数 poll
     * @param fds 系统poll_fd结构体数组
     * @param nfds 关注的套接字最大值
     * @param timeout 最长等待时间, 毫秒
     * @return >0 准备就绪的套接字个数, <0 失败
     */
    static int poll(struct pollfd fds[], nfds_t nfds, int timeout);

    static short epollevent2poll( int events );
    ...
    int SetFdReadTimeout(int fd,int timeout);

    int SetFdWriteTimeout(int fd,int timeout);
...
private:
     ...
     * @brief 微线程私有构造
     */
    MtFrame():_realtime(1){ _curr_thread = NULL; _waitnum = 0;}; 
