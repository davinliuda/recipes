...
#define FREQ_LIMIT_BEGIN(x)	   do {\
										int64_t _now_ts = __spp_get_now_ms();\
										static int64_t _next_log_ts = 0;	\
										if(_next_log_ts < _now_ts)\
										{\
											_next_log_ts = _now_ts+(x); 
 										

#define FREQ_LIMIT_END     		    }\
							 		}while(0);
