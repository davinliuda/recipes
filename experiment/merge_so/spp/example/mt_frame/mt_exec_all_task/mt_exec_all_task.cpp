...
public:
    virtual int Process() {
        ...
        server_addr.sin_port = htons(5574);

        buf_size = 1024;
        int ret = mt_udpsendrcv(&server_addr, (void*)buff, send_size, buff, buf_size, 100);  /* 任务里面使用UDP一发一收微线程接口 */
        if (ret < 0)
        {
...
int ExampleMsg::HandleProcess()
{
    ...
    memcpy(task1.buff, (char*)_msg_buff.data(), (int)_msg_buff.size());
    task1.send_size = (int)_msg_buff.size();
    memcpy(task2.buff, (char*)_msg_buff.data(), (int)_msg_buff.size());
    task2.send_size = (int)_msg_buff.size();
    ...
    blob_type blob;
    char concat_buf[2048];
    memcpy(concat_buf, task1.buff, task1.buf_size);
    memcpy(&concat_buf[task1.buf_size], task2.buff, task2.buf_size);
    
    blob.data = concat_buf;
    blob.len  = task1.buf_size + task2.buf_size;
